...
export default function Signup() {
  ...
  // assuming you already have these state setters + values:
  // email, password, confirmPassword, setError, setLoading, setEmailConfirmationRequired
  setError(null);
  ...
  try {
    ...
    // 1) Create Supabase auth user (treat everyone as owner for now)
    const { data, error: signUpError } = await supabase.auth.signUp({
      email,
      password,
      options: {
        data: {
          role: 'owner', // MVP: everyone is an owner
        },
      },
    });
    ...
    // 2) Handle “confirm email” mode (no session yet)
    ...
    // 3) Best-effort profile upsert (NON-blocking)
    ...
    // 4) Go straight into the onboarding wizard
